...
using std::vector;
using std::pow;  //sneo
int avtFLASHXFileFormat::objcnt = 0;
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::Block::Print
//
//  Programmer:  Jeremy Meredith
...
// ****************************************************************************
void
avtFLASHXFileFormat::Block::Print(ostream &out)
{
    out << "---- BLOCK: "<<ID<<endl;
...
// ****************************************************************************
//  Function:  avtFLASHXFileFormat::InitializeHDF5
//
...
// ****************************************************************************
void
avtFLASHXFileFormat::InitializeHDF5(void)
{
...
// ****************************************************************************
//  Function:  avtFLASHXFileFormat::FinalizeHDF5
//
...
// ****************************************************************************
void
avtFLASHXFileFormat::FinalizeHDF5(void)
{
...
avtFLASHXFileFormat::avtFLASHXFileFormat(const char *cfilename, 
    ...
    : avtSTMDFileFormat(&cfilename, 1)
{
    ...
    if (avtFLASHXFileFormat::objcnt == 0)
        InitializeHDF5();
    avtFLASHXFileFormat::objcnt++;
}


// ****************************************************************************
//  Function:  avtFLASHXFileFormat::~avtFLASHXFileFormat
//
...
//
// ****************************************************************************
avtFLASHXFileFormat::~avtFLASHXFileFormat()
{
    FreeUpResources();
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::ActivateTimestep
//
//  Purpose:
...
void
avtFLASHXFileFormat::ActivateTimestep(void)
{
    BuildDomainNesting();
}

// ****************************************************************************
//  Method: avtFLASHXFileFormat::GetCycle
//
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::FreeUpResources
//
//  Purpose:
...
void
avtFLASHXFileFormat::FreeUpResources(void)
{
    if (fileId >= 0)
    {
        H5Fclose(fileId);
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::PopulateDatabaseMetaData
//
//  Purpose:
...
void
avtFLASHXFileFormat::PopulateDatabaseMetaData(avtDatabaseMetaData *md)
{
    ReadAllMetaData();
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::GetMesh
//
//  Purpose:
...
//    Added support for FLASHX3 formats.
//
//    Randy Hudson, Apr 3, 2007
//    Added support for Morton curve.
//
...
avtFLASHXFileFormat::GetMesh(int domain, const char *meshname)
{
    ...
    else if (string(meshname) == "particles")
    {
        ...
        if (fileFormatVersion < FLASHX3_FFV8)
        {
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::GetMortonCurveSubset
...
vtkPolyData *
avtFLASHXFileFormat::GetMortonCurveSubset(int domain)
{
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::GetMortonCurve
//
//  Purpose:
...
vtkPolyData *
avtFLASHXFileFormat::GetMortonCurve()
{
    vtkPolyData *pdata = vtkPolyData::New();
    vtkPoints *points = vtkPoints::New();
    vtkCellArray *lines = vtkCellArray::New();
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::GetVar
//
//  Purpose:
...
vtkDataArray *
avtFLASHXFileFormat::GetVar(int visitDomain, const char *vname)
{
    ReadAllMetaData();
    ...
    if (pos != string::npos)
        ...
        theRealDomain = visitIdToFLASHXId[visitDomain];
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::GetVectorVar
//
//  Purpose:
...
vtkDataArray *
avtFLASHXFileFormat::GetVectorVar(int visitDomain, const char *varname)
{
    // Is this necessary?  No.  But I don't want someone to get caught if
    // they ever decide to implement this method.
    //   int domain = visitIdToFLASHXId[visitDomain];
    return NULL;
}


// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadAllMetaData
//
//  Purpose:
...
void
avtFLASHXFileFormat::ReadAllMetaData()
{
    ...
    if (fileFormatVersion < FLASHX3_FFV8)
    {
        ReadParticleAttributes(); // FLASHX2 version
    }
    else 
    {
        ReadParticleAttributes_FLASHX3(); // FLASHX3 version
    }
    ...
    if (numBlocks > 0)
    {
        ...
        visitIdToFLASHXId.resize(numBlocks, -1);
        vector<int> numForThisLevel(numLevels, 0);
        for (i = 0 ; i < numBlocks ; i++)
        {
             int level = blocks[i].level-1;
             int visitIdx = numToThisPoint[level] + numForThisLevel[level]++;
             visitIdToFLASHXId[visitIdx] = i;
        }

        FLASHXIdToVisitId.resize(numBlocks, -1);
        for (i = 0 ; i < numBlocks ; i++)
        {
             FLASHXIdToVisitId[visitIdToFLASHXId[i]] = i;
        }
    }
}
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::ReadProcessorNumbers
//
//  Purpose:
...
void avtFLASHXFileFormat::ReadProcessorNumbers()
{
    hid_t rootId = H5Gopen(fileId, "/");
    if (rootId < 0)
    {
        debug5 << "[avtFLASHXFileFormat::ReadProcessorNumbers] - Didn't open root group" << endl;
        H5Fclose(fileId);
        EXCEPTION1(InvalidFilesException, filename.c_str());
    }
    hsize_t num_obj;
    ...
    if (errId < 0)
    {
        debug5 << "[avtFLASHXFileFormat::ReadProcessorNumbers] - Can't get # of objects in root group" << endl;
        H5Gclose(rootId);
        H5Fclose(fileId);
        EXCEPTION1(InvalidFilesException, filename.c_str());
    }
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::ReadCoordinates
//
//  Purpose:
...
//
// ****************************************************************************
void avtFLASHXFileFormat::ReadCoordinates()
{
    //
    ...
    else if (fileFormatVersion == FLASHX3_FFV9)
    {
        if (coordinates_ndims != 2 ||
            coordinates_dims[0] != (hsize_t)numBlocks ||
...
// ****************************************************************************
//  Method: avtFLASHXFileFormat::ReadNodeTypes
//
//  Purpose:
...
void avtFLASHXFileFormat::ReadNodeTypes()
{
    //
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadBlockStructure
//
//  Purpose:
...
//    Let this function fail silently; some FLASHX files contain only
//    particles and no grids, and we want to support those without errors.
//
// ****************************************************************************

void avtFLASHXFileFormat::ReadBlockStructure()
{
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadBlockExtents
//
//  Purpose:
...
//
// ****************************************************************************
void avtFLASHXFileFormat::ReadBlockExtents()
{
    //
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadRefinementLevels
//
//  Purpose:
...
//
// ****************************************************************************
void avtFLASHXFileFormat::ReadRefinementLevels()
{
    //
    // Read the bounding box description for the blocks
    //
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadSimulationParameters
//
//  Purpose:
...
//    flavors of FLASHX files have stored the data members in different
//    order or as different data types, and thus we can't assume a single
//    data structure.
//
//    Kathleen Bonnell, Thu Jul 20 11:22:13 PDT 2006
//    Added support for FLASHX3 file versions.
//
//    Mark C. Miller, Tue Mar  6 23:41:33 PST 2007
//    Added bool for reading just enough to get cycle/time info
//
// ****************************************************************************
void avtFLASHXFileFormat::ReadSimulationParameters(hid_t file_id,
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadUnknownNames
//
//  Purpose:
...
void
avtFLASHXFileFormat::ReadUnknownNames()
{
    //
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadParticleAttributes
//
//  Purpose:
...
//    FLASHX files containing only particles and no grid data.
//
...
// ****************************************************************************
void
avtFLASHXFileFormat::ReadParticleAttributes()
{
    // temporarily disable error reporting
    H5E_auto_t  old_errorfunc;
    void       *old_clientdata;
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::DetermineGlobalLogicalExtentsForAllBlocks
//
//  Purpose:
...
//
// ****************************************************************************
void avtFLASHXFileFormat::DetermineGlobalLogicalExtentsForAllBlocks()
{
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::BuildDomainNesting
//
//  Purpose:
...
void
avtFLASHXFileFormat::BuildDomainNesting()
{
    ...
    if ((*vrTmp == NULL))
    {
        ...
        if (numLevels > 0)
        {
            ...
            // NOTE: FLASHX files always have a 2:1 ratio
            vector<int> ratios(3);
            ratios[0] = 1;
            ratios[1] = 1;
            ratios[2] = 1;
            ...
            for (i = 0; i < numBlocks; i++)
            {
                ...
                for (int j = 0; j < numChildrenPerBlock; j++)
                {
                    ...
                    if (blocks[i].childrenIDs[j] >= 0)
                        childBlocks.push_back(FLASHXIdToVisitId[blocks[i].childrenIDs[j] - 1]);
                }
                ...
                dn->SetNestingForDomain(FLASHXIdToVisitId[i], blocks[i].level-1,
                                        childBlocks, logExts);
            ...
            cache->CacheVoidRef("amr_mesh",
                                AUXILIARY_DATA_DOMAIN_NESTING_INFORMATION,
                                timestep, -1, vr);
            visitTimer->StopTimer(t2, "FLASHX setting up patch nesting");
        }
        ...
        if (addStructuredDomainBoundaries)
        {
            ...
            for (i = 0; i < numBlocks; i++)
            {
                ...
                logExts[5] = blocks[i].maxGlobalLogicalExtents[2];
                rdb->SetIndicesForAMRPatch(FLASHXIdToVisitId[i], blocks[i].level - 1, logExts);
            }
            ...
            cache->CacheVoidRef("any_mesh", AUXILIARY_DATA_DOMAIN_BOUNDARY_INFORMATION,
                                timestep, -1, vrdb);
            visitTimer->StopTimer(t1, "FLASHX setting up domain boundaries");
        }
    }
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::GetAuxiliaryData
//
//  Purpose:
...
// ****************************************************************************
void *
avtFLASHXFileFormat::GetAuxiliaryData(const char *var, int dom, 
                                     ...
                                     DestructorFunction &df)
{
    ...
    for (int b = 0 ; b < numBlocks ; b++)
    {
        ...
        itree->AddElement(FLASHXIdToVisitId[b], bounds);
    }
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadVersionInfo
//
//  Purpose:
...
void
avtFLASHXFileFormat::ReadVersionInfo(hid_t file_id)
{
    debug5 << "Determining FLASHX file format version." << endl;
    // temporarily disable error reporting
    H5E_auto_t  old_errorfunc;
    void       *old_clientdata;
    H5Eget_auto(&old_errorfunc, &old_clientdata);
    ...
    // If this is a FLASHX3 Particles file, or a FLASHX3 file with particles, 
    ...
    {
        flash3_particles = 1;
        debug5 << " Found particle names, assuming FLASHX3; need to test for file format version 8 or 9" << endl;
        H5Dclose(h5_PN);
    }

    //
    // Read the file format version  (<= 7 means FLASHX2)
    //
    ...
    if (h5_FFV < 0)
    {
        ...
        if (h5_SI < 0)
        {
            debug5 << "sim info not found, assuming FLASHX2...unless FLASHX3 particles" << endl;
            if (flash3_particles == 1)
            {
                fileFormatVersion = FLASHX3_FFV8;
            }
            else
            {
                fileFormatVersion = 7;
    ...
    {
        // FLASHX 2 has file format version available in global attributes.
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadIntegerScalars
//
//  Purpose:
...
void 
avtFLASHXFileFormat::ReadIntegerScalars(hid_t file_id)
{
    ...
    if (intScalarsId < 0)
    {
        debug5 <<  "FLASHX3  could not read integer scalars"<< endl;
        EXCEPTION1(InvalidFilesException, filename.c_str());
    }
    ...
    if (spaceId < 0)
    {
        debug5 << "FLASHX3 could not get the space of integer scalars" << endl;
        EXCEPTION1(InvalidFilesException, filename.c_str());
    }
	...
	//sneo: need below
        ...
        else if (strncmp(is[i].name, "igridsize", 9) == 0)
            simParams.igridsize = is[i].value;
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadRealScalars
//
//  Purpose:
...
avtFLASHXFileFormat::ReadRealScalars(hid_t file_id)
{
    ...
    if (realScalarsId < 0)
    {
        debug5 << "FLASHX3 could not read real scalars" << endl;
        EXCEPTION1(InvalidFilesException, filename.c_str());
    }
    ...
    if (spaceId < 0)
    {
        debug5 << "FLASHX3 could not get the space of real scalars" << endl;
        EXCEPTION1(InvalidFilesException, filename.c_str());
    }
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadParticleAttributes_FLASHX3
//
//  Purpose:
...
void
avtFLASHXFileFormat::ReadParticleAttributes_FLASHX3()
{
    // Should only be used for FLASHX3 files
    ...
    if (pnameId < 0)
    {
        debug5 << "FLASHX3 no particles names" << endl;
        numParticles = 0; 
        return;
    }
    ...
    if (ndims != 2 || dims[1] != 1) 
    {
        if (ndims != 2)
        {
            debug5 << "FLASHX3 expecting particle names ndims of 2, got "  
                   << ndims << endl;
        }
        if (dims[1] != 1)
        {
            debug5 << "FLASHX3 expecting particle names dims[1] of 1, got "  
                   << dims[1] << endl;
        }
        EXCEPTION1(InvalidFilesException, filename.c_str());
    }
    ...
    if (pointId < 0)
    {
        debug5 << "FLASHX3 no tracer particles" << endl;
        numParticles = 0;
        return;
    }
    ...
    if (p_ndims != 2)
    {
        debug5 << "FLASHX3, expecting particle tracer ndims of 2, got"
               << p_ndims << endl;
        EXCEPTION1(InvalidFilesException, filename.c_str());
    }
    numParticles = p_dims[0];
...
// ****************************************************************************
//  Method:  avtFLASHXFileFormat::ReadParticleVar
//
//  Purpose:
...
void
avtFLASHXFileFormat::ReadParticleVar(hid_t pointId, const char *vname, 
    double *ddata)
{
    // Should only be used for FLASHX3 files
